...
class PolygoinIOGateway:
    def __init__(self):
        self.session = requests.session()
        self.base_url = constants.POLYGOIN_IO_BASE_URL
        self.session.headers['Content-Type'] = 'Application/json'
        self.session.headers['Authorization'] = constants.POLYGON_IO_AUTH_KEY


    def make_api_call(self, endpoint:str, method:str,params:dict, payload:dict):
        LOGGER.debug("Initializing api call for Polygon_IO api gateway")
        method = method.upper()
